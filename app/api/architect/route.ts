...
const PROVIDERS: any = {
  ...
  groq:   { url: "https://api.groq.com/openai/v1", model: "llama3-70b-8192" } // Groq is the fastest for real-time
};
...
export async function POST(req: Request) {
  try {
    ...
    let focusContext = isGlobal 
      ? "SCOPE: GLOBAL REFACTOR. Modify ANY and ALL files." 
      : (activeFile ? `SCOPE: FOCUSED on "${activeFile.path}".` : "SCOPE: Project Overview.");

    const lockedFiles = currentFileTree.filter((f: any) => f.locked_by && f.locked_by !== userId).map((f: any) => f.path);

    // 2. Stream-Optimized Prompt (XML format instead of JSON)
    const systemPrompt = `
...
`;
    ...
    const client = new OpenAI({ apiKey, baseURL: selectedProvider.url });

    const completion = await client.chat.completions.create({
      model: selectedProvider.model,
      messages: [{ role: "system", content: systemPrompt }, ...chatContext, { role: "user", content: prompt }],
      stream: true, // ENABLING REAL-TIME STREAMING
    });

    // 3. Pipe Stream to Client & Save to DB in background
    const stream = new ReadableStream({
      async start(controller) {
        ...
        while ((match = fileRegex.exec(fullResponse)) !== null) {
            ...
            // Final lock check
            const { data: fileCheck } = await supabase.from("files").select("locked_by").eq("project_id", projectId).eq("path", path).single();
            if (fileCheck?.locked_by && fileCheck.locked_by !== userId && userRole !== "Leader") continue;

            await supabase.from("files").upsert({
              project_id: projectId, path: path, content: content,
